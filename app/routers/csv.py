...
from app.services.company_csv_service import CompanyCSVService
    ...
    **Note:** Column names can be in Portuguese or English. The system accepts only standard terms:
    ...
    try:
        ...
        for c in customers_created:
            try:
                customer_resp = CustomerResponse.from_customer(c)
...
@router.post("/customers/validate", response_model=Dict)
...
@router.post("/companies/upload", response_model=Dict)
async def upload_companies_csv(
    file: UploadFile = File(...),
    skip_invalid: bool = Query(True, description="If True, only valid rows are saved. If False, returns error if any row is invalid"),
    return_invalid_details: bool = Query(True, description="If True, returns detailed information about invalid rows"),
    skip_duplicates: bool = Query(True, description="If True, skips duplicate companies (by CNPJ, name, or portal_id). If False, returns error if duplicates found")
):
    """
    Uploads and processes a CSV file with company data.
    ...
    - cnpj: Company CNPJ
    - email: Company email
    - phone/telefone: Company phone
    - address/endereco: Company address
    ...
    - employee_count/quantidade_funcionarios: Number of employees
    - license_type/tipo_licenca: 'Start' or 'Hub'
    - portal_id: Company ID in the License Portal
    - notes/notas: Additional notes
    
    **Note:** Column names can be in Portuguese or English. The system accepts standard terms.
    
    Query Parameters:
    - skip_invalid: If True (default), only valid rows are saved. If False, returns error if any row is invalid.
    - return_invalid_details: If True (default), returns detailed information about invalid rows.
    - skip_duplicates: If True (default), skips duplicate companies (by CNPJ, name, or portal_id). If False, returns error if duplicates found.
    
    Returns processing statistics, list of created companies, validation details, and duplicate information.
    """
    if not file.filename.endswith('.csv'):
        raise HTTPException(status_code=400, detail="File must be a CSV")
    
    try:
        logger.info(f"Starting Company CSV upload: {file.filename} (size: {file.size if hasattr(file, 'size') else 'unknown'})")
        ...
        # Processes CSV
        logger.info("Processing Company CSV file...")
        result = await CompanyCSVService.process_csv(file_bytes)
        validation: CSVValidationResult = result.get("validation")
        
        logger.info(f"CSV processed: {result['success']} valid companies, {validation.invalid_rows} invalid rows")
        ...
        # If no valid companies, return error
        if not result["companies"]:
            logger.warning(f"No valid companies found. Total errors: {validation.invalid_rows}")
            return {
                "success": False,
        ...
        # Check for duplicates
        logger.info(f"Checking for duplicates among {len(result['companies'])} companies...")
        duplicates = await CompanyRepository.check_duplicates(result["companies"])
        ...
        for company in result["companies"]:
            ...
            # Check by CNPJ
            if company.cnpj and company.cnpj.strip() and company.cnpj in duplicates:
                is_duplicate = True
                existing_company = duplicates[company.cnpj]
                duplicate_reason = f"CNPJ {company.cnpj} already exists"
            
            # Check by name (if not already found by CNPJ)
            elif company.name in duplicates:
                is_duplicate = True
                existing_company = duplicates[company.name]
                duplicate_reason = f"Name '{company.name}' already exists"
            
            # Check by portal_id (if not already found)
            elif company.portal_id and company.portal_id.strip() and company.portal_id in duplicates:
                is_duplicate = True
                existing_company = duplicates[company.portal_id]
                duplicate_reason = f"Portal ID {company.portal_id} already exists"
            ...
            if is_duplicate:
                duplicate_details.append({
                    ...
                    "reason": duplicate_reason,
                    "existing_company_id": str(existing_company.id) if existing_company else None,
                    "existing_company_name": existing_company.name if existing_company else None
                })
                logger.debug(f"Duplicate company skipped: {company.name} - {duplicate_reason}")
            else:
                companies_to_create.append(company)
        
        # If skip_duplicates is False and there are duplicates, return error
        if not skip_duplicates and duplicates:
            logger.warning(f"CSV has duplicate companies and skip_duplicates=False. Returning error.")
            return {
                "success": False,
                "message": f"CSV contains {len(duplicates)} duplicate company/companies. Use skip_duplicates=true to skip duplicates.",
                "duplicates": duplicate_details,
                "total_rows": result["total_rows"],
                "valid_rows": validation.valid_rows,
                "duplicate_count": len(duplicates)
        ...
        # If no companies to create after filtering duplicates
        if not companies_to_create:
            logger.warning(f"All companies are duplicates. Total duplicates: {len(duplicates)}")
            return {
                "success": False,
                "message": "All companies in CSV are duplicates",
                "duplicates": duplicate_details if return_invalid_details else None,
                "duplicate_count": len(duplicates),
                "total_rows": result["total_rows"]
            }
        
        # Creates companies in database (only non-duplicates)
        logger.info(f"Creating {len(companies_to_create)} new companies in database (skipping {len(duplicates)} duplicate(s))...")
        try:
            companies_created = await CompanyRepository.create_many(companies_to_create)
            logger.info(f"Companies created successfully: {len(companies_created)}")
        except Exception as db_error:
            logger.error(f"Error creating companies in database: {type(db_error).__name__}: {db_error}")
            logger.error(f"Error details: {str(db_error)}", exc_info=True)
            raise HTTPException(
                status_code=500,
                detail=f"Error saving companies to database: {str(db_error)}"
            )
        
        # Converts to response
        logger.debug("Converting companies to response format...")
        companies_response = []
        for c in companies_created:
            try:
                ...
                companies_response.append(company_resp)
            except Exception as conv_error:
                logger.error(f"Error converting company to response: {type(conv_error).__name__}: {conv_error}")
                logger.error(f"Problematic company - ID: {c.id if hasattr(c, 'id') else 'N/A'}, Name: {c.name if hasattr(c, 'name') else 'N/A'}")
                raise
        
        logger.info(f"CSV upload completed successfully: {len(companies_created)} companies created")
        
        # Build message
        message_parts = [f"Successfully created {len(companies_created)} companies"]
        if validation.has_errors:
            message_parts.append(f"{validation.invalid_rows} rows were invalid and skipped")
        if duplicates:
            message_parts.append(f"{len(duplicates)} duplicate(s) were skipped")
        ...
        if duplicates:
            ...
            if return_invalid_details:
                ...
                response["duplicates_summary"] = f"{len(duplicates)} company/companies were duplicates and skipped"
...
@router.post("/companies/validate", response_model=Dict)
async def validate_companies_csv(
    file: UploadFile = File(...),
    return_invalid_details: bool = Query(True, description="If True, returns detailed information about invalid rows")
):
    """
    Validates a Company CSV file without saving to database.
    
    This endpoint only validates the CSV structure and data, but does not create any companies.
    Useful for checking CSV validity before actual import.
    ...
    **Note:** Column names can be in Portuguese or English. The system automatically normalizes them.
    ...
    try:
        logger.info(f"Validating Company CSV file: {file.filename}")
